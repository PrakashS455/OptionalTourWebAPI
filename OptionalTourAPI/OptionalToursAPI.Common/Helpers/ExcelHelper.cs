...
namespace PAMAPI.Common.Helpers
{
    public class ExcelHelper
    {
        ...
        public static List<T> ConvertWorksheetToList<T>(ExcelWorksheet excelWorksheet, Dictionary<string, string> map = null) where T : new()
        {
            //DateTime Conversion
            ...
            {
                var cellValue = (excelWorksheet.Cells[startRow, col].Value ?? string.Empty).ToString().Trim();
            ...
            for (int rowIndex = startRow + 1; rowIndex <= endRow; rowIndex++)
            {
                ...
                {
                    var value = excelWorksheet.Cells[rowIndex, column.Index].Value;
                    ...
                    if (prop != null)
                    {
                        ...
                        else if (propertyType == typeof(TimeSpan?) || propertyType == typeof(TimeSpan))
                        {
                            ...
                            if (double.TryParse(valueStr, out val))
                            {
                                DateTime dt = DateTime.FromOADate((double)val);
                                parsedValue = new TimeSpan(dt.Hour, dt.Minute, dt.Second);
                            }
                            else
                            {
                                parsedValue = val;
